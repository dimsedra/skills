...
10. **Continuous Pedagogical Preference Memory & Override Authority**: Before generating a lesson, inspect the workspace root for `.reporting-preferences.md`. If found, apply all learned pedagogical preferences (analogy styles, code focus areas, diagram types) and **allow it to override default CSS variables or HTML section templates**. Never create a blank preference file beforehand; only create or update `.reporting-preferences.md` when the user provides feedback on explanation delivery or learning style.
